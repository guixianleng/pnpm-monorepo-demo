  ...
  const delAllVisitedViews = (): Promise<RouteLocationNormalized[]> => {
    return new Promise(resolve => {
      visitedViews.value = visitedViews.value.filter(
        (tag: RouteLocationNormalized) => tag.meta?.affix
      )
      resolve([...visitedViews.value])
    })
  }
  ...
  const delRightTags = (view: RouteLocationNormalized): Promise<RouteLocationNormalized[]> => {
    return new Promise(resolve => {
      const index = visitedViews.value.findIndex(
        (v: RouteLocationNormalized) => v.path === view.path
      )
      if (index === -1) {
        return
      }
      visitedViews.value = visitedViews.value.filter(
        (item: RouteLocationNormalized, idx: number) => {
