...
import { deepClone, isNumber } from "advint-ui"
...
const values = computed(() => {
  ...
  if (Array.isArray(props.value)) {
    return props.value.map(item => (props.numberToString ? `${item}` : item))
  } else {
    const oldValue = deepClone(props.value)
    if (isNumber(props.value)) {
      return props.numberToString ? String(oldValue).split(props.separator) : [props.value]
    }
