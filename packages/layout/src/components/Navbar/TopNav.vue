    ...
    <el-sub-menu v-if="topMenus.length > visibleNumber" :style="{ '--theme': theme }" index="more">
      ...
      <template v-for="(item, index) in topMenus">
        <el-menu-item v-if="index >= visibleNumber" :key="index" :index="item.path">
          <adv-svg-icon :icon-class="item.meta ? item.meta.icon : ''" />
          {{ item.meta?.title }}
        </el-menu-item>
      </template>
    </el-sub-menu>
...
import { isHttp } from "@user-admin/utils"
...
.topmenu-container.el-menu--horizontal > .el-menu-item {
  height: $base-top-nav-height !important;
  line-height: $base-top-nav-height !important;
...
/* 图标右间距 */
.topmenu-container .adv-svg-icon {
  margin-right: 4px;
}
</style>
